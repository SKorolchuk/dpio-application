...
describe('HttpService', () => {
      ...
      providers: [
        ...
        HttpCacheService,
        {
          ...
          useFactory: (
            ...
            return new HttpService(backend, defaultOptions, configuration, cacheService);
          },
          deps: [MockBackend, BaseRequestOptions, ConfigurationService, HttpCacheService]
        }
      ]
    });
  });
  ...
  beforeEach(inject(
    [HttpService, HttpCacheService, MockBackend],
    (_httpService: HttpService, _httpCacheService: HttpCacheService, _mockBackend: MockBackend) => {
  ...
  describe('request', () => {
    it('should prepend environment.appEndpoint to the request url', fakeAsync(() => {
      // Act
      ...
      // Assert
      expect(lastConnection.request.url).toEqual(environment.appEndpoint + '/toto');
    }));

    it('should prepend environment.appEndpoint to the request object url property', fakeAsync(() => {
      // Act
  ...
  describe('get', () => {
    it('should call request with GET method', () => {
      ...
      // Assert
      expect((<jasmine.Spy>httpService.request).calls.mostRecent().args).toEqual([
        '/toto',
  ...
  describe('post', () => {
    it('should call request with POST method and body', () => {
      ...
      expect((<jasmine.Spy>httpService.request).calls.mostRecent().args).toEqual([
        '/toto',
        { method: RequestMethod.Post, body: { tata: 123 } }
      ]);
    });
  });
  ...
  describe('put', () => {
    it('should call request with PUT method and body', () => {
      ...
      expect((<jasmine.Spy>httpService.request).calls.mostRecent().args).toEqual([
        '/toto',
        { method: RequestMethod.Put, body: { tata: 123 } }
      ]);
    });
  });
  ...
  describe('patch', () => {
    it('should call request with PATCH method and body', () => {
      ...
      expect((<jasmine.Spy>httpService.request).calls.mostRecent().args).toEqual([
        '/toto',
        { method: RequestMethod.Patch, body: { tata: 123 } }
      ]);
    });
  });
