...
export class UserManagementEffects {
    ...
    registerRedirect$ = this.actions$.pipe(
        ofType<fromAuth.RegisterSuccess>(
            fromAuth.RegisterActionTypes.RegisterSuccess
        ),
        exhaustMap(registerSuccessAction => {
            log.debug(`successfully registered`);
            this.router.navigate([this.userService.loginUrl]);
            return of(
                new fromAuth.RegisterComplete(registerSuccessAction.payload)
            );
        })
    );
    ...
    registerErrorRedirect$ = this.actions$.pipe(
        ofType<fromAuth.RegisterFailure>(
            fromAuth.RegisterActionTypes.RegisterFailure
        ),
    ...
    resetPassword$ = this.actions$.pipe(
        ofType<fromAuth.ResetPassword>(
            fromAuth.ResetPasswordActionTypes.ResetPassword
        ),
        exhaustMap(resetPasswordAction => {
            return this.userService
                .resetPassword(resetPasswordAction.payload)
                .pipe(
                    map((resetPasswordResult: any) => {
                        if (resetPasswordResult) {
                            return new fromAuth.ResetPasswordSuccess(
                                resetPasswordResult
                            );
                        }
                    }),
                    catchError(error =>
                        of(new fromAuth.ResetPasswordFailure(error))
                    )
                );
        })
    );
    ...
    resetPasswordRedirect$ = this.actions$.pipe(
        ofType<fromAuth.ResetPasswordSuccess>(
            fromAuth.ResetPasswordActionTypes.ResetPasswordSuccess
        ),
        exhaustMap(resetPasswordSuccessAction => {
            log.debug(`successfully reset password`);
            this.router.navigate([this.userService.loginUrl]);
            return of(
                new fromAuth.ResetPasswordComplete(
                    resetPasswordSuccessAction.payload
                )
            );
        })
    );
    ...
    resetPasswordErrorRedirect$ = this.actions$.pipe(
        ofType<fromAuth.ResetPasswordFailure>(
            fromAuth.ResetPasswordActionTypes.ResetPasswordFailure
        ),
        map(action => action.payload),
        tap((err: any) => {
            if (err.error_description) {
