...
function buildCommand(command, locale, output, needFile) {
    ...
    if (needFile) {
        command +=
            ` --i18n-file=apps/dpio-application/src/i18n/messages.${locale.toLowerCase()}.xlf --i18n-format=xlf`;
    }
    return command;
}
...
for (let i = 0; i < locales.length; i++) {
    ...
    console.info(folder);
    execSync(buildCommand(baseCommand, locales[i], folder, project.locales[locales[i]]), {
        cwd: process.cwd(),
        stdio: [0, 1, 2]
    });
    console.info();
